...
class PaypalController extends Controller
{
    ...
    public function expressCheckout(Request $request) {
        ...
        if (!$response['paypal_link']) {
          return redirect('/')->with('error', 'Something went wrong with PayPal');
      ...
      private function getCart($recurring, $invoice_id)
        {
            ...
            return [
                ...
                'items' => [
                    [
                        'name' => 'Product 1',
                        'price' => config('paypal.deduct_amount'),
                        'qty' => 1,
                    ],
                ],
                ...
                'total' => config('paypal.deduct_amount'),
            ];
        }
        ...
        public function expressCheckoutSuccess(Request $request) {
            ...
            // find invoice by id
            $invoice = Invoice::where('order_id',$invoice_id)->first();
            ...
            if ($invoice->paid) {
                $amount = config('paypal.deduct_amount');
                $msg = "Your order ( #$invoice_id ) has been placed successfully! and $".$amount." has been paid!";
